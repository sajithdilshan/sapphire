...
describe Feed do 
  ...
  describe 'userFeedUpdate method' do
    before :each do
  ...
  describe 'AddFeed Method ' do 

    describe 'Before fetching feed ' do
      it 'should check if the feed already exists' do
        Feed.should_receive(:check_if_feed_exists_by_feed_url).with("feed_url")
        Feed.stub(:check_if_feed_exists_by_feed_url).and_return(@fetched_feed)
      end
      it 'should call Feedzirra with the feed url if feed does not exist' do 
        Feed.stub(:check_if_feed_exists_by_feed_url).and_return(nil)
    ...
    it 'should return alert if the fectching feed was a failiure' do
      ...
      Feed.addFeed("feed_url","userid").should == {alert: "Error occured while fetching feed. Please try again later."}
    end
    ...
    it 'should create userfeed for current user and return sucess if feed already exists' do
      ...
      User.stub(:find_by_uid).and_return(@user)
      Feed.addFeed("feed_url","userid") == {notice: "Feed added Successfully"}
    end
    ...
    it 'should return an error message if creating feed was unsuccessful' do
      ...
      Feed.addFeed("feed_url","userid") == {alert: "Error occured while creating feed. Please try again later."}
    end
